...
describe("Vote", () => {
  ...
  beforeEach((done) => {
    // user, topic, post, vote to be used
    this.user;
    this.topic;
    this.post;
    this.vote;

    // clear the database and create the above 4 items
  ...
  describe("#create()", () => {
    it("should create an upvote on a post for a user", (done) => {
      // create an upvote for this.user on this.post
      Vote.create({
        value: 1,
        postId: this.post.id,
        userId: this.user.id
      ...
      .then((vote) => {
        ...
        // check the vote was created successfully
    ...
    // create a downvote for this.user on this.post
    it("should create a downvote on a post for a user", (done) => {
      ...
      .then((vote) => {
        ...
        // check the downvote was created successfully
        expect(vote.value).toBe(-1);
        expect(vote.postId).toBe(this.post.id);
        expect(vote.userId).toBe(this.user.id);
        done();
